...
// sees raw accounts, only the finished figures. Throws on any failure so the
// panel can render its own error copy.
export async function postNarrate(metrics: unknown, lang: Lang): Promise<Brief> {
  ...
  const brief = data as Partial<Brief>;
  if (!brief || typeof brief.headline !== "string" || !Array.isArray(brief.actions)) {
    throw new Error("The narrator returned an unexpected shape.");
  }
  return {
    headline: brief.headline,
    summary: typeof brief.summary === "string" ? brief.summary : "",
    actions: brief.actions.filter(
      (a): a is BriefAction => Boolean(a) && typeof a.title === "string"
    ),
  };
}
